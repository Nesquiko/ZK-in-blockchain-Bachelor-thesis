// Source: https://github.com/verumlotus/ZK-Hashes/tree/main
import bigInt, { BigInteger } from "big-integer";
import { BN128_PRIME, poseidonConstants } from "./constants";
...
function buildPoseidon(prime: string) {
  ...
  function poseidon(inputs: bigint[]) {
    if (inputs.length > N_ROUNDS_P.length) {
      throw new Error("to many inputs");
    }
...
export function poseidon(inputs: bigint[]): bigint {
  if (inputs.length <= 0 || inputs.length > 16) {
    throw new Error(
      `Inputs array was too large, it must have a length less than or equal to 16`,
    );
  }
  const _poseidonFunction = buildPoseidon(BN128_PRIME);
